...
class fetchcomplainStudentPage extends StatefulWidget {
  const fetchcomplainStudentPage({Key? key}) : super(key: key);

  @override
  fetchcomplainStudentPageState createState() =>
      fetchcomplainStudentPageState();
}

class fetchcomplainStudentPageState extends State<fetchcomplainStudentPage> {
  // to get all the documents in collection
  final Stream<QuerySnapshot> studentsStream =
      FirebaseFirestore.instance.collection('Tbl_ComplaintPortal').snapshots();

  @override
  Widget build(BuildContext context) {
    final Stream<QuerySnapshot> studentsStream = FirebaseFirestore.instance
        .collection('Tbl_ComplaintPortal')
        .snapshots();
                        ...
                        for (var i = 0; i < storedocs.length; i++) ...[
                          DataRow(
                            cells: [
                              ...
                              DataCell(Text(storedocs[i]['Complain'],
                                  style: const TextStyle(fontSize: 18.0))),
                              DataCell(Container(
                                  child: Text(storedocs[i]['Complain_Date'],
                                      style: const TextStyle(fontSize: 18.0)))),
                            ],
                          ),
                        ],
